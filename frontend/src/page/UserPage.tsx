...
const UserPage = () => {
  ...
  const handleSubmit = (
    ...
    if (users.selected) {
      ...
      dispatch(startAddUser(values, action, toast));
    }
  };

  useEffect(() => {
    dispatch(startGetUserAll());
  }, [dispatch]);
    ...
    >
      <Card>
        <Formik
          ...
          validationSchema={
            users.selected ? userUpdateValidator : userAddValidator
          }
        >
  ...
  return (
    <Form>
      ...
      <Flex flexDir={{ base: "column", lg: "row" }} gap={2}>
        ...
        />
        {users.selected && (
          <Button title="Cancelar" onClick={() => handleOfSelected()} />
        )}
      </Flex>
    </Form>
  );
