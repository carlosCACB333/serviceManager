...
const TableItem = ({ ticket, idx, mode = 'all' }: { ticket: TicketInterface; idx: number; mode?: 'all' | 'short' }) => {
  ...
  return (
    <Tr key={ticket.id}>
      ...
      <Td isNumeric>
        {estado <= 0 ? <Badge colorScheme="green">Cancelado</Badge> : <Badge colorScheme="red">- S/{estado} </Badge>}
      </Td>
      <Td textAlign="center">
        {ticket.finish_date ? (
        ...
        <Menu>
          ...
          <MenuList>
            <Link to={'/ticket/' + ticket.id}>
              <MenuItem icon={<BsFillArrowUpRightSquareFill />}>Detalle de la venta</MenuItem>
            </Link>
          </MenuList>
        </Menu>
      </Td>
